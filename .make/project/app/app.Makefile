...
_deptrac_run_baseline:
	...
	@-${_DC_EXEC} ${APP_CONTAINER} deptrac analyse --clear-cache --config-file=${DPTR_CONFIG} --cache-file=${DPTR_CACHE} > ${DPTR_OUT_DIR_LOCAL}/${_DEPTRAC_REPORT_FILE}
...
app_deptrac_run:
	@${_ECHO};
	@-${_DC_EXEC} ${APP_CONTAINER} deptrac analyse --no-progress --config-file=${DPTR_CONFIG} --cache-file=${DPTR_CACHE}
