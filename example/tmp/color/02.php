...
foreach (C256_C_RAINBOW as $colorCode) {
    ...
    $equality = $ansi8stripped === $colorCode ? '==' : '!=';

    $csi = str_replace(ESC, '\e', CSI);
    $reset = str_replace(ESC, '\e', RESET);

    echo sprintf(
        '%s    FA: %s    Ansi4: %s     Ansi8: %s     Ansi24: %s   %s' . PHP_EOL,
        TABLE[$colorCode],
        $csi . "38;5;{$colorCode}mC" . $reset,
        sprintf($csi . '3%smC' . $reset, $ansi4,),
        sprintf($csi . '3%smC' . $reset, $ansi8,),
        sprintf($csi . '3%smC' . $reset, $ansi24,),
        sprintf('%s%s%s', $ansi8stripped, $equality, $colorCode)
